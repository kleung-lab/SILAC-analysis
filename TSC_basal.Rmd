...
```{r repeat with all proteins, fig.height = 6, fig.width=10}
# 
# skypro.TSC1 <- skypro[skypro$exp_ref == 'TSC1',]
# # 
# # mouse <- UniProt.ws(taxId=10090)
...
# # topPathways.up <- gsea_out[head(order(-NES), n=15)][,pathway]
# 
# topPathways.up <- gsea_out[ES > 0][head(order(pval), n=10), pathway]
# topPathways.down <- gsea_out[ES < 0][head(order(pval), n=10), pathway]
# #TOP table
# plotGseaTable(Mm.KL[topPathways], pro.stat, gsea_out, gseaParam=0.5)
# #top upregulated
# plotGseaTable(Mm.KL[topPathways.up], pro.stat, gsea_out, gseaParam=0.5)
...
```{r compare to RAD001}
library(reshape2)
skypro_rad <- read.csv("../TSC_RAD001/TSC_RAD001_processed_TSC1.csv")
skypro_comp <- rbind(skypro_rad, skypro)
compare <- dcast(skypro_comp[skypro_comp$enrich.exp.p > 1.3,], Protein.Accession + Protein.Gene ~ exp_ref, value.var = "enrich.exp.med")
p <- ggplot(compare, aes(TSC1, TSC1_RAD001)) + geom_jitter() + theme_classic()
library(ggrepel)
p + geom_label_repel(data = compare[compare$TSC1_MEF > 1 & compare$TSC1_RAD001 < -1,], aes(label = Protein.Gene), fontface = "bold")
p
```

# 102 cells
Compare to 102 cell lines provided by Lisa Henske
```{r 102}
# HOMmap <- read.csv("~/Box Sync/MS/MSpeaklist/lib/HOM_MouseHumanSequence20180124.rpt", sep = "\t")
# load()
```
